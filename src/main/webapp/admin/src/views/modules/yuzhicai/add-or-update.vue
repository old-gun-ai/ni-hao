	...
	<div class="addEdit-block" :style='{"padding":"30px 30px 0"}' style="width: 100%;">
		...
		>
			<template >
				...
				<el-form-item :style='{"width":"45%","margin":"0 2% 20px 0","display":"inline-block"}' class="upload" v-if="type!='info' && !ro.caipintupian" label="菜品图片" prop="caipintupian">
					<file-upload
						tip="点击上传菜品图片"
						action="file/upload"
						:limit="3"
				...
				<el-form-item :style='{"width":"45%","margin":"0 2% 20px 0","display":"inline-block"}' class="upload" v-else-if="ruleForm.caipintupian" label="菜品图片" prop="caipintupian">
				...
				<el-form-item :style='{"width":"45%","margin":"0 2% 20px 0","display":"inline-block"}' class="input" v-if="type!='info'"  label="制作时长" prop="zhizuoshizhang">
					<el-input v-model="ruleForm.zhizuoshizhang" placeholder="制作时长" clearable  :readonly="ro.zhizuoshizhang"></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"45%","margin":"0 2% 20px 0","display":"inline-block"}' v-else class="input" label="制作时长" prop="zhizuoshizhang">
					<el-input v-model="ruleForm.zhizuoshizhang" placeholder="制作时长" readonly></el-input>
				</el-form-item>
				...
				<el-form-item :style='{"width":"45%","margin":"0 2% 20px 0","display":"inline-block"}' class="textarea" v-if="type!='info'" label="菜品简介" prop="caipinjianjie">
					<el-input
				...
				<el-form-item :style='{"width":"45%","margin":"0 2% 20px 0","display":"inline-block"}' v-else-if="ruleForm.caipinjianjie" label="菜品简介" prop="caipinjianjie">
					<span :style='{"fontSize":"14px","lineHeight":"40px","color":"rgba(90, 67, 74, 1)","fontWeight":"500","display":"inline-block"}'>{{ruleForm.caipinjianjie}}</span>
				</el-form-item>
...
export default {
    ...
    info(id) {
      this.$http({
        ...
        this.ruleForm.caipinxiangqing = this.ruleForm.caipinxiangqing.replace(reg,'../../../ssmgmpms/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `yuzhicai/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.yuzhicaiCrossAddOrUpdateFlag = false;
						 this.parent.search();
